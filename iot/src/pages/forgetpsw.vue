        ...
        <div class="login-page" v-show="pager === 1">
          ...
          <div class="login-group">
            <div class="login-input">
              <span style="color: #8590a6">验证码将会发送至你的注册邮箱或手机</span>
            </div>
          </div>
          <!--<form>-->
            <div class="login-group">
              <div class="login-input">
                <el-input size="small" placeholder="注册手机号或电子邮箱" v-model.trim="forgetForm.username" @keyup.enter.native="sendCode"></el-input>
              </div>
            </div>
            <div class="login-group">
    ...
    methods: {
      ...
      sendCode () {
        if (this.forgetForm.username) {
          ...
          //  账号是手机号
          if (mobileReg.test(this.forgetForm.username)) {
            this.ismobile = true
            this.$ajax.get("/forgetpsw/caihiot/tcust/sendcode", {
              params: {
          ...
          //  账户是邮箱
          } else if (emailReg.test(this.forgetForm.username)) {
            this.ismobile = false
            this.$ajax.get("/forgetpsw/caihiot/tcust/sendcode", {
              params: {
                custAccount: this.forgetForm.username,
          ...
          this.$message.error("请输入注册手机号或邮箱")
        }
      },
      //验证验证码
      vailMailCode () {
        if (this.forgetForm.code && this.forgetForm.code.length === 6) {
          this.$ajax.get("/forgetpsw/caihiot/tcust/vailcode", {
            params: {
      ...
      resetPsw () {
        if (this.forgetForm.password && this.forgetForm.passwordAgain) {
          if (this.forgetForm.password === this.forgetForm.password) {
            this.$ajax.post("/forgetpsw/caihiot/tcust/resetPsw", {
